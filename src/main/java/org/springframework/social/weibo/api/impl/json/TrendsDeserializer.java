...
public class TrendsDeserializer extends JsonDeserializer<SortedSet<Trends>> {
    ...
    public SortedSet<Trends> deserialize(JsonParser jp,
        ...
        Iterator<String> stringIterator = jp.readValueAsTree().fieldNames();
        while (stringIterator.hasNext()) {
            String fieldName = stringIterator.next();
            Trends trends = new Trends();
            try {
                dateFormat
                        .applyPattern(retrieveDateFormatPattern(fieldName));
                trends.setDate(dateFormat.parse(fieldName));
                JsonNode trendsNode = (JsonNode) jp.readValueAsTree().get(fieldName);
                for (Iterator<JsonNode> iterator2 = trendsNode.elements(); iterator2
                        .hasNext(); ) {
